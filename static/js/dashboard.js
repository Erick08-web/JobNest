document.addEventListener('DOMContentLoaded', () => {
    ...
    function showAuthModal() {
        const modalElement = document.getElementById('modalNoAutenticado');
    ...
    async function initializeDashboard() {
        ...
        if (isAuthenticated) {
            ...
            userType = 'cliente';
            userData = { nombres: 'Invitado', correo: '', apellido_paterno: '', apellido_materno: '', telefono: '' };
        ...
        if (isAuthenticated) {
            ...
            mostrarMensajeNoAutenticadoEnSecciones();
        }
    }

    function mostrarMensajeNoAutenticadoEnSecciones() {
        const containers = ['publicaciones-list', 'solicitudes-list', 'servicios-populares', 'resultados-busqueda', 'mis-solicitudes-body', 'servicios-concluidos-container', 'solicitudes-pago-container'];
        containers.forEach(id => {
            const el = document.getElementById(id);
            if (el) {
                el.innerHTML = `<div class="col-12"><div class="card glass-card text-center p-4"><i class="bi bi-lock display-4"></i><p class="mt-3">Inicia sesión para ver y gestionar esta sección.</p><a href="/templates/login.html" class="btn btn-primary">Iniciar sesión</a></div></div>`;
            }
        });
    }
    ...
    async function fetchUserData() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                updateUIWithUserData(userData);
                actualizarFotoNavbar(userData.foto_perfil);
            } else {
    ...
    function setupUserInterface() {
        ...
        if (userType === 'prestador') {
            if (prestadorSidebar) prestadorSidebar.classList.remove('d-none');
            if (clienteSidebar) clienteSidebar.classList.add('d-none');
            showSection('inicio-prestador');
        } else {
            if (clienteSidebar) clienteSidebar.classList.remove('d-none');
            if (prestadorSidebar) prestadorSidebar.classList.add('d-none');
            showSection('inicio-cliente');
        }
        const userTypeBadge = document.getElementById('userTypeBadge');
        if (userTypeBadge) userTypeBadge.textContent = userType;
        const dropdownUserType = document.getElementById('dropdownUserType');
        if (dropdownUserType) dropdownUserType.textContent = userType;
    }

    function updateUIWithUserData(userData) {
        const fullName = `${userData.nombres || ''} ${userData.apellido_paterno || ''} ${userData.apellido_materno || ''}`.trim();
        const navbarUserName = document.getElementById('navbarUserName');
        if (navbarUserName) navbarUserName.textContent = fullName || 'usuario';
        const dropdownUserName = document.getElementById('dropdownUserName');
        if (dropdownUserName) dropdownUserName.textContent = fullName || 'usuario';
        const dropdownUserEmail = document.getElementById('dropdownUserEmail');
        if (dropdownUserEmail) dropdownUserEmail.textContent = userData.correo || 'correo@ejemplo.com';
        if (userType === 'prestador') {
            const welcomeMsg = document.getElementById('welcomeMessagePrestador');
            if (welcomeMsg) welcomeMsg.textContent = `¡Bienvenido/a, ${userData.nombres || 'prestador'}!`;
        } else {
            const welcomeMsg = document.getElementById('welcomeMessageCliente');
            if (welcomeMsg) welcomeMsg.textContent = `¡Bienvenido/a, ${userData.nombres || 'cliente'}!`;
        }
        const profileName = document.getElementById('profileName');
        if (profileName) profileName.value = userData.nombres || '';
        const profileLastNameP = document.getElementById('profileLastNameP');
        if (profileLastNameP) profileLastNameP.value = userData.apellido_paterno || '';
        const profileLastNameM = document.getElementById('profileLastNameM');
        if (profileLastNameM) profileLastNameM.value = userData.apellido_materno || '';
        const profileEmail = document.getElementById('profileEmail');
        if (profileEmail) profileEmail.value = userData.correo || '';
    ...
    async function cargarFotoPerfil() {
        try {
            const response = await fetch('/obtener_foto_perfil', { credentials: 'same-origin' });
            ...
            if (result.success && result.foto_url) {
                ...
                if (avatarImg) {
                    ...
                    avatarImg.src = result.foto_url + '?t=' + Date.now();
                }
                actualizarFotoNavbar(result.foto_url);
            } else {
                if (placeholder) placeholder.style.display = 'flex';
                if (avatarImg) avatarImg.style.display = 'none';
                actualizarFotoNavbar(null);
            }
        } catch (error) {
    ...
    function actualizarFotoNavbar(fotoUrl) {
    const placeholder = document.getElementById('navbarAvatarPlaceholder');
    const avatarImg = document.getElementById('navbarAvatarImg');
    if (!placeholder || !avatarImg) return;
    if (fotoUrl) {
        placeholder.style.display = 'none';
        avatarImg.style.display = 'block';
        avatarImg.src = fotoUrl + '?t=' + Date.now();
    } else {
    ...
    function setupFotoPerfil() {
        ...
        fileInput.addEventListener('change', async (e) => {
            ...
            if (!allowed.includes(file.type)) {
                showToast('Formato no permitido. Use JPG, PNG o GIF', 'error');
                return;
            }
            ...
            try {
                const response = await fetch('/subir_foto_perfil', {
                    method: 'POST',
                    body: formData,
                ...
                if (result.success) {
                    ...
                    if (avatarImg) avatarImg.src = result.foto_url + '?t=' + Date.now();
                    actualizarFotoNavbar(result.foto_url);
                    if (userData) userData.foto_perfil = result.foto_url;
                } else {
                                ...
                                <div class="card glass-card">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center mb-2">
                                            ${sol.cliente_foto ?
                                                `<img src="${sol.cliente_foto}" class="avatar-small rounded-circle" alt="cliente">` :
                                                `<div class="avatar-small rounded-circle d-flex align-items-center justify-content-center bg-primary text-white" style="width:40px;height:40px;"><i class="bi bi-person-circle fs-4"></i></div>`
                            ...
                            <div class="col-md-4">
                                <div class="card glass-card h-100">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center mb-2">
                                            ${serv.prestador_foto ?
                                                `<img src="${serv.prestador_foto}" class="avatar-small rounded-circle" alt="foto">` :
                                                `<div class="avatar-small rounded-circle d-flex align-items-center justify-content-center bg-primary text-white" style="width:40px;height:40px;"><i class="bi bi-person-circle fs-4"></i></div>`
                                            }<h5 class="card-title mb-0">${serv.titulo}</h5>
                                        </div>
                        ...
                        container.innerHTML = publicaciones.map(serv => `
                            <div class="col-md-6">
                                <div class="card glass-card h-100">
                                    <div class="card-body">
                                        <div class="d-flex align-items-center mb-2">
                                            <img src="${serv.prestador_foto || '/static/default-avatar.png'}" class="avatar-small rounded-circle" alt="foto">
                                            <h5 class="card-title mb-0">${serv.titulo}</h5>
                                        </div>
                                        <p class="text-muted">${serv.categoria}</p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        ${sol.prestador_foto ?
                                            `<img src="${sol.prestador_foto}" class="rounded-circle" width="30" height="30" style="object-fit:cover; margin-right:8px;">` :
                                            `<div class="rounded-circle d-inline-flex align-items-center justify-content-center bg-primary text-white" style="width:30px;height:30px; margin-right:8px;"><i class="bi bi-person-circle"></i></div>`
                                        }${sol.prestador_nombre}
                                    </div>
                                </td>
                ...
                if (container) {
                    if (result.conversaciones.length === 0) {
                        ...
                        container.innerHTML = result.conversaciones.map(conv => `
                            <div class="conversacion-item" data-hilo="${conv.id}" data-solicitud="${conv.solicitud_id}" data-otro="${conv.otro_nombre}" data-foto="${conv.otro_foto || ''}">
                                ${conv.otro_foto ?
                                    `<img src="${conv.otro_foto}" class="conversacion-avatar-img rounded-circle">` :
                                    `<div class="conversacion-avatar-img rounded-circle d-flex align-items-center justify-content-center bg-primary text-white" style="width:48px;height:48px;"><i class="bi bi-person-circle fs-3"></i></div>`
                                }<div class="conversacion-info">
                                    <h6>${conv.titulo_publicacion}</h6>
                                    <p><strong>${conv.otro_nombre}</strong> ${conv.ultimo_mensaje ? conv.ultimo_mensaje.substring(0, 50) : ''}</p>
                        ...
                        document.querySelectorAll('.conversacion-item').forEach(item => {
                            item.addEventListener('click', () => {
                                ...
                                const otroFoto = item.dataset.foto;
                                cargarMensajes(hiloId, solicitudId, otroNombre, otroFoto);
                            });
                        });
                    }
                }
    ...
    async function cargarMensajes(hiloId, solicitudId, otroNombre, otroFoto) {
        if (!requireAuth()) return;
        try {
            ...
            if (response.ok && result.success) {
                document.getElementById('chat-header').innerHTML = `
                    <div class="d-flex align-items-center">
                        ${otroFoto ?
                            `<img src="${otroFoto}" class="avatar-chat rounded-circle">` :
                            `<div class="avatar-chat rounded-circle d-flex align-items-center justify-content-center bg-primary text-white" style="width:40px;height:40px;"><i class="bi bi-person-circle fs-4"></i></div>`
                        }<h6 class="mb-0">Conversación con ${otroNombre}</h6>
                    </div>
                `;
